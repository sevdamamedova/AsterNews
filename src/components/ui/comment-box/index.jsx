...
function CommentBox({ items = [], onSubmit, onDelete, btnLoading = false }) {
  ...
  return (
    <>
          ...
          <form onSubmit={handleSubmit}>
            <div>
              <textarea
                ...
                className="form-element dark:!bg-gray-800 dark:border-gray-700 !bg-[#ECF5F8] !py-3"
                cols="30"
                rows="4"
              ></textarea>
            </div>
      ...
      {!token && (
        <div className="my-[35px]">
          <Alert type="warning">Rəy bildirmək üçün sayta daxil olun</Alert>
        </div>
      )}
      <div>
        <div>
          <button
            ...
            className="text-[#2F9FF8] text-[17px] inline-flex items-center gap-x-2"
          >
            <span>View All Comments ({items.length})</span>
            <span className="inline-flex size-[23px] items-center justify-center bg-[#2F9FF8] text-white rounded-full">
              ...
              {items.map((item, index) => (
                <div key={index}>
                  <div className="flex gap-x-3">
                    ...
                    <div>
                      <h5 className="text-[#2F9FF8] text-[17px]">
                  ...
                  <div className="text-[#ADBCC4] mt-[5px] flex items-center justify-between">
                    <div>
                      <span className="text-[12px]">{moment(item.created_at).format("DD-MM-YYYY HH:mm")}</span>
                    </div>
                    <div>
